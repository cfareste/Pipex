...
/*   Updated: 2024/03/08 18:00:19 by cfidalgo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_child_processes(t_state *state)
{
	...
	i = FIRST_COMMAND_INDEX + state->here_doc;
	...
	if (close(STDIN_FILENO) == -1)
		exit(handle_error(NULL));
	return (i - (FIRST_COMMAND_INDEX + state->here_doc));
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (argc < 5 || (has_here_doc(argv[HERE_DOC_INDEX]) && argc < 6))
